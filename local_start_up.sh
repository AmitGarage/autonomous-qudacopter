...
while true; do
    if tail -n 100 ${BASE_DIR}/log/term1.log | grep -F "INFO  [uxrce_dds_client] successfully created rt/fmu/out/gimbal_device_attitude_status"; then
...
if [[ "$input" == "e" ]]; then
    ...
    ps -ef | grep -E "gz|ros2|8888|px4" | grep -v grep | awk '{print $2}' | xargs kill -${2:-'TERM'}
